...
class pieChartFin extends baseModal {
    constructor() {
        var config = {
            id: "pieChartFin",
...
counts <- table({{dataset.name}}[,'{{selected.pieVar | safe}}'])
counts_df <- as.data.frame(counts)
names(counts_df) <- c("{{selected.pieVar | safe}}", "Count")

counts_df$Percentage <- round(prop.table(counts) * 100, digits = BSkyGetDecimalDigitSetting())
...
ggplot(data={{dataset.name}}, aes({{if (options.selected.x[0] == "")}}x='', {{#else}}{{selected.x[0] | safe}}{{/if}}{{selected.y[0] | safe}}{{selected.color[0] | safe}} )) +
    ...
    coord_polar("y") +
    geom_text(aes(label = paste({{selected.pieVar | safe}}, "\n", "(", Percentage, "%)")), position = position_stack(vjust = 0.5)) +
...
module.exports.item = new pieChartFin().render()